...
export default function Invoices() {
  ...
  const filteredInvoices = invoiceRecords
    ...
    .sort((a, b) => {
      const dueDateA = unixToDate(
        getField(a.recordPlaintext, "due_date") || "0u64",
      ).getTime();
      const dueDateB = unixToDate(
        getField(b.recordPlaintext, "due_date") || "0u64",
      ).getTime();
      const cmp =
        sortBy === "amount"
          ? microToAleo(getField(a.recordPlaintext, "amount") || "0u64") -
            microToAleo(getField(b.recordPlaintext, "amount") || "0u64")
          : dueDateA - dueDateB;
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {isLoading
                ? "Loading…"
                : `${factoredRecords.length} factored invoice${factoredRecords.length !== 1 ? "s" : ""}`}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
